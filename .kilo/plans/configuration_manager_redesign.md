...
- [ ] Add `RegistryConfiguration` contract type with `docker_login` validation strategy
- [ ] Add `GitHubCredentials` contract type with `github_api_check` validation strategy
- [ ] Update `build-ci-image.yaml` to resolve contracts via Configuration Manager instead of GitHub secrets
- [ ] Update CI worker image entrypoint to fetch validated config
- [ ] Remove `.env` reading from CI workflow steps
- [ ] Integration test: invalid credentials cause immediate pipeline failure with clear error
