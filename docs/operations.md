...
4. **Config** — confirm the git tag at the promoted SHA + encrypted `.env` copy exist.
...
`openobserve_storage`, `minio_data`, `n8n_data`, `gitea_data`, `registry_data`, `dind_data`, `langgraph_config`.

## Secrets

`.env` holds all secrets. It is **never** committed; an encrypted copy is part of the backup set.
Rotate via the documented procedure in `runbook-recovery.md` and update registry credentials.
