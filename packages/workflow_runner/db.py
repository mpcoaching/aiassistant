...
def _pg_upsert_instance(state: WorkflowState, database_url: str) -> None:
    try:
...
def _pg_insert_step_result(step_result: dict[str, Any], workflow_id: str, step_index: int, workflow_path: str = "", database_url: str = "") -> None:
    try:
        with _pg_conn(database_url) as conn:
            with conn.cursor() as cur:
                cur.execute(
...
def _pg_insert_event(event_id: str, workflow_id: str, event_type: str, payload: dict[str, Any], database_url: str) -> None:
    try:
        with _pg_conn(database_url) as conn:
            with conn.cursor() as cur:
                cur.execute(
...
def _pg_upsert_schedule(schedule: dict[str, Any], database_url: str) -> None:
    try:
        with _pg_conn(database_url) as conn:
            with conn.cursor() as cur:
                cur.execute(
...
def _pg_delete_schedule(schedule_id: str, database_url: str) -> None:
    try:
        with _pg_conn(database_url) as conn:
            with conn.cursor() as cur:
...
def create_workflow_state(
    ...
    _persist_state(state, database_url=database_url)
    return state


def _persist_state(state: WorkflowState, database_url: str | None = None) -> None:
    _persist_file(state)
    if database_url:
        try:
...
def load_workflow_state(workflow_id: str, workflow_path: str, database_url: str | None = None) -> WorkflowState | None:
    # Try Postgres first
    if database_url:
        try:
            with _pg_conn(database_url) as conn, conn.cursor() as cur:
...
def list_workflow_states(workflow_path: str, database_url: str | None = None) -> list[dict[str, Any]]:
...
def update_workflow_state(state: WorkflowState, database_url: str | None = None) -> None:
    _persist_state(state, database_url=database_url)


def advance_step(state: WorkflowState, result: StepResult, database_url: str | None = None) -> WorkflowState:
...
def fail_workflow(state: WorkflowState, error: str, database_url: str | None = None) -> WorkflowState:
    state.status = "failed"
    state.error = error
    _persist_state(state, database_url=database_url)
    return state
...
def pause_workflow(state: WorkflowState, reason: str = "user_requested", database_url: str | None = None) -> WorkflowState:
    state.status = "paused"
    _persist_state(state, database_url=database_url)
    return state


def resume_workflow(state: WorkflowState, database_url: str | None = None) -> WorkflowState:
    state.status = "running"
    _persist_state(state, database_url=database_url)
    return state


def stop_workflow(state: WorkflowState, reason: str = "user_requested", database_url: str | None = None) -> WorkflowState:
    state.status = "stopped"
    _persist_state(state, database_url=database_url)
    return state


def append_log(state: WorkflowState, message: str, database_url: str | None = None) -> None:
    _append_file_log(state, message)
    if database_url:
        _pg_insert_event(str(uuid.uuid4())[:8], state.workflow_id, "log", {"message": message}, database_url)


def record_step_result(state: WorkflowState, result: StepResult, step_index: int, database_url: str | None = None) -> None:
    _pg_insert_step_result(result.model_dump(), state.workflow_id, step_index, state.workflow_path, database_url=database_url or "")


def insert_event(event_id: str, workflow_id: str, event_type: str, payload: dict[str, Any], database_url: str | None = None) -> None:
    if database_url:
        _pg_insert_event(event_id, workflow_id, event_type, payload, database_url)


def get_schedules(database_url: str | None = None) -> list[dict[str, Any]]:
    if database_url:
        return _pg_get_schedules(database_url)
    return []


def upsert_schedule(schedule: dict[str, Any], database_url: str | None = None) -> None:
    if database_url:
        _pg_upsert_schedule(schedule, database_url)


def delete_schedule(schedule_id: str, database_url: str | None = None) -> None:
    if database_url:
        _pg_delete_schedule(schedule_id, database_url)
