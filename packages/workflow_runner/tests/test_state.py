...
class TestLoadWorkflowState:
    def test_load_existing_state(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            wf_path = str(Path(tmpdir) / "test.yaml")
            created = create_workflow_state("test", wf_path, _make_steps(), database_url="")

            loaded = load_workflow_state(created.workflow_id, wf_path, database_url="")
            assert loaded is not None
            assert loaded.workflow_id == created.workflow_id
            assert loaded.workflow_name == created.workflow_name

    def test_load_nonexistent_state(self):
        loaded = load_workflow_state("nonexistent", "/tmp/fake.yaml", database_url="")
...
class TestAdvanceStep:
    def test_advance_to_next_step(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            wf_path = str(Path(tmpdir) / "test.yaml")
            state = create_workflow_state("test", wf_path, _make_steps(), database_url="")
            ...
            state = advance_step(state, result, database_url="")
...
class TestFailWorkflow:
    def test_fail_workflow(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            state = fail_workflow(state, "Something went wrong", database_url="")
...
class TestListStates:
    def test_list_states(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            wf_path = str(Path(tmpdir) / "test.yaml")
            create_workflow_state("test1", wf_path, _make_steps(), database_url="")
            create_workflow_state("test2", wf_path, _make_steps(), database_url="")

            states = list_workflow_states(wf_path, database_url="")
            assert len(states) == 2
